...
3. When plucking a ~Mu p ~ p (Mu p)~ we end up with two choices:
   ...
   1. We receive a fruit ~Mu p ~ p (Mu p)~ and a dissection ~q v (Mu p)~. We have to process this fruit
      as we did in step 2, and store the dissection somewhere until we can fill it with the
      processed fruit.
...
some indirection as to not perform implicit left-recursion on ~cata~; see the implementation in
